 ...
 * the Transformation interface and defines common logic for applying pixel-wise operations on an
 * image. The class is abstract and intended to be extended by specific transformation operations.
 */
public abstract class AbstractBasicOperation implements Transformation {
   ...
   * transformed image. The transformation is performed on a pixel-by-pixel basis, producing a new
   * image where each pixel's color is computed based on the original pixel values.
   *
   * @param key the key used to identify the image in the {@code h1} HashMap
   * @param h1  a {@code HashMap} containing image data, where the key is a string identifier and
   *            the value is a 2D array of {@code RGBPixel} objects representing the image's pixels
   ...
   * intended to be overridden by subclasses to provide specific behavior for different operations.
   *
