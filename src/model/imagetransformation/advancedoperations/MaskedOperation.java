...
/**
 * The {@code MaskedOperation} class implements the {@link Transformation} interface and applies a specified image
 * transformation to a source image based on a mask. The mask determines which pixels in the source image should
 * be modified by the transformation operation. If the corresponding mask pixel is black (0, 0, 0), the operation
 * is applied; otherwise, the original pixel value is retained.
 */
public class MaskedOperation implements Transformation {
   ...
   * @param mask a 2D array of {@link Pixels} representing the mask used to determine which pixels to transform
   */
  ...
  /**
   * Applies the masked transformation operation to the provided source image pixels.
   * The operation is applied only to the pixels in the source image where the corresponding
   * mask pixel is black (0, 0, 0). Other pixels in the source image remain unchanged.
   *
   * @param sourcePixels a 2D array representing the source image pixels
   * @return a 2D array of pixels representing the resulting image after the masked operation is applied
   * @throws IllegalArgumentException if the dimensions of the mask do not match the dimensions of the source pixels
   */
  @Override
