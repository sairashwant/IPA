 ...
 * It adjusts the color balance of the image by analyzing the histograms of the
 * red, green, and blue channels, finding the peak values for each channel, and
 * adjusting the color values to match a target peak. This process helps to
 * balance the image's colors based on the distribution of pixel values.
 *
 * <p>The correction is achieved by shifting the peaks of each channel towards
 * a target peak, which is the average of the peak values from the red, green,
 * and blue channels. The resulting image has adjusted color balance based on
 * the histogram data.</p>
 */
public class ColorCorrection implements Transformation {
   ...
   * by considering values in the range [10, 245] to avoid extreme outliers. The peak
   * represents the most common color value for a given channel.</p>
   *
   ...
   * @return Histogram array of size 256 representing the frequency of pixel values for the specified channel.
   * @throws IllegalArgumentException if the pixel data is null or if any pixel is not an instance of {@link RGBPixel}.
   */
   ...
   * computes a target peak as the average of these peaks, and adjusts the pixel values
   * accordingly. Each channel is adjusted by the difference between the target peak and
   * the peak for that channel. The corrected pixel values are then clamped to the valid
   * range [0, 255] to ensure valid RGB values.</p>
   *
   * @param pixels The 2D array of {@link Pixels} representing the image to be corrected.
   * @return A 2D array of corrected {@link RGBPixel} objects representing the adjusted image pixels.
   * @throws IllegalArgumentException if the pixel data is null or if any pixel is not an instance of {@link RGBPixel}.
   */
  @Override
