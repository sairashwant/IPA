...
/**
 * This class represents a basic image transformation operation that brightens the pixels
 * of an image by a specified factor. It extends the AbstractBasicOperation class.
 */
public class  Brighten extends AbstractBasicOperation {
   ...
   *
   * @param brightenFactor The amount by which to brighten each pixel's color values.
   *                      This value can be positive (to increase brightness) or negative (to decrease brightness).
   */
  ...
  /**
   * Applies the brighten operation to the specified image pixels.
   * It retrieves the pixel data using the provided key from the HashMap,
   * brightens each pixel by the brighten factor, and returns the modified pixel array.
   *
   * @param key The key used to retrieve the image pixel data from the HashMap.
   * @param h1  A HashMap containing image pixel data, where the key is a String and the value is a 2D array of RGBPixel objects.
